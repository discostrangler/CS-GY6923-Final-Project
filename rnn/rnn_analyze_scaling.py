...
LOG_PREFIX = "rnn_"  # expects files like rnn_tiny.out, rnn_small.out, ...


def parse_log(model_name: str):
    """
    Parse a rnn_<model>.out file and extract metrics.
    """
    log_path = BASE_DIR / f"{LOG_PREFIX}{model_name}.out"
    ...
    n_params = grab(r"RNN .*has\s+([\d,]+)\s+trainable parameters", int, allow_commas=True)
    if n_params is None:
        # fallback: generic "Parameters:" line, if present
...
def build_summary_and_write_csv():
    """
    Parse all RNN logs, fit power law, and write rnn_scaling_summary.csv
    ...
    print(f"RNN fitted power law: L ≈ {fit_a:.4f} * N^(-{fit_alpha:.4f}) + {fit_c:.4f}")
    print(f"RNN scaling exponent alpha ≈ {fit_alpha:.4f}")
    ...
    summary_path = BASE_DIR / "rnn_scaling_summary.csv"
    fieldnames = [
        "model_name",
        "n_params",
        "final_val_loss",
    ...
    print("Wrote RNN summary with fit parameters to", summary_path)
    for r in rows:
        print(
            f"rnn_{r['model_name']:6s}  params={r['n_params']:,}  "
...
def make_scaling_plot(rows, fit_a, fit_alpha, fit_c):
    """
    Create rnn_scaling_plot.png with log X axis and linear Y axis.
    ...
    plt.title(f"RNN scaling (alpha ≈ {fit_alpha:.3f})")
    plt.tight_layout()
    out_path = BASE_DIR / "rnn_scaling_plot.png"
    plt.savefig(out_path, dpi=200)
    plt.close()
    print("Saved RNN scaling plot to", out_path)


def parse_training_curve(model_name: str):
    """
    Parse 'Step X/Y  loss Z  lr ...' lines from rnn_<model>.out
...
def make_training_curves_all():
    """
    Plot all RNN training loss curves in a single figure.
    ...
    for m in MODELS:
        ...
        if not steps:
            print(f"No training curve data found for rnn_{m}, skipping.")
            continue
    ...
    if not any_data:
        print("No RNN training curve data for any model. Not writing combined plot.")
        plt.close()
        return

    plt.xlabel("Step")
    plt.ylabel("Training loss")
    plt.title("Training loss curves for all RNN sizes")
    plt.legend()
    plt.tight_layout()
    png_path = BASE_DIR / "rnn_train_curves_all.png"
    plt.savefig(png_path, dpi=200)
    plt.close()
    print(f"Wrote combined RNN training curves plot to {png_path}")
